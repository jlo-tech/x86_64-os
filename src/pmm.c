...
void ktree_insert(struct ktree_root *root, void *foreign_selector, i64 selector_offset, i64 (*compare)(void *c, void *f))
{
    ...
    if(!root->valid)
    {
        root->root = (struct ktree_node*)(foreign_selector+selector_offset);
    }
    else
    {
        ...
        while(1) {
            ...
            i64 c = compare((void*)(node+selector_offset), foreign_selector);
            // Handle left first
            if(c < 0)
            {
                ...
                {
                    // If not insert
                    node->left = (struct ktree_node*)(foreign_selector-selector_offset);
                    // And set the parent node pointer
                    ((struct ktree_node*)(foreign_selector-selector_offset))->parent = node;
                    // Done
                    return;
                }
            }
            ...
            else
            {
                ...
                {
                    // If not insert
                    node->right = (struct ktree_node*)(foreign_selector-selector_offset);
                    // And set the parent node pointer
                    ((struct ktree_node*)(foreign_selector-selector_offset))->parent = node;
                    // Done
                    return;
...
void ktree_remove(struct ktree_root *root, void *foreign_selector, i64 selector_offset, i64 (*compare)(void *c, void *f))
{
    ...
    if(compare((void*)(root+selector_offset), foreign_selector) == 0)
    {
        root->valid = false;
    }
    else
